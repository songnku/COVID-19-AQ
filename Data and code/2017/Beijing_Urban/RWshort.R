...
registerDoFuture()
plan(multiprocess)
filenamelist<-list("Beijing_Urban")
#polllist<-list("pm2_5","pm10","pm25_pm10")
polllist<-list("no2","co","o3","so2")
ncal=100
...
RF_model <- rmw_do_all(
  data_prepared,
  variables = c(
    "date_unix","day_julian", "weekday","hour", "temp",  "RH", "wd", "ws","sp","cluster","tp","blh","tcc","ssr"),variables_sample=c("temp",  "RH", "wd", "ws","sp","cluster","tp","blh","tcc","ssr"),
  n_trees = 300,
  n_samples = 300,
  verbose = TRUE
)
